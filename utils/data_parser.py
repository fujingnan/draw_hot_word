...
class Parser(KeyItermMining):
    def __init__(self, is_lower=True, window=2, print_num=20, word_min_len=2, min_phrase_len=2, topic_num=3,
                 tag_filter=util.allow_speech_tags, filepath='', current_day=20220402):
        """
    ...
    def transform_map(self, is_current):
        ...
        sids = type_day.id.tolist()
        symbols = type_day.stock_name.tolist()
        titles = type_day.title.tolist()
        texts = type_day.text.tolist()
        symbol_text_map = dict()
        for i in range(len(type_day)):
            title = str(titles[i]) + '。' if str(titles[i]) else ''
            content = title + str(texts[i])
            ...
            symbol_text_map[symbol].setdefault('id', []).append(sid)
            symbol_text_map[symbol].setdefault('text', []).append(content)
    ...
    def get_text_info(self, extract_level, texts):
        ...
        """
        if not extract_level in {'keywords', 'keysentence', 'alls'}:
            return "please choose one of parameters in {'keywords', 'keysentence', 'alls'}"
    ...
    def main(self, extract_level):
        ...
        :return: (curr_symbol_text_map, his_symbol_text_map)Dict: {
            symbol: {
                'extract_results': 文本解析结果(同 get_text_info 方法返回格式)
            }
        }
        """
        ...
        for symbol, item in tqdm(curr_symbol_text_map.items()):
            ...
            curr_symbol_text_map[symbol]['extract_results'] = extract_results
        print("解析历史文本信息……")
        for symbol, item in tqdm(his_symbol_text_map.items()):
            texts = item['text']
            extract_results = self.get_text_info(extract_level, texts)
            his_symbol_text_map[symbol]['extract_results'] = extract_results
        return curr_symbol_text_map, his_symbol_text_map

if __name__ == '__main__':
    parser = Parser(filepath='../csv/hot_stock_cards.csv')
    cur_res, his_res = parser.main('keywords')
    print(his_res)